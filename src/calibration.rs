...
pub fn run_calibration(spec_path: &Path, corpus_path: &Path) -> Result<CalibrationReport, Box<dyn std::error::Error>> {
    ...
    let mut check_freqs: std::collections::HashMap<String, CheckFrequency> = std::collections::HashMap::new();
    for check_def in &spec.checks {
        check_freqs.insert(check_def.id.clone(), CheckFrequency {
            check_id: check_def.id.clone(),
            category: check_def.category.clone(),
            pass_count: 0, fail_count: 0, manual_count: 0, error_count: 0,
            total_documents: 0,
            fail_documents: vec![],
            fail_details: vec![],
        });
            ...
            if let Some(freq) = check_freqs.get_mut(&result.check_id) {
                ...
                match result.status {
                    ...
                    Status::Fail => {
                        ...
                        freq.fail_details.push(format!("[{}] {}", filename, result.detail));
                    }
    ...
    Ok(CalibrationReport {
        ...
        documents: pdf_files.iter().map(|e| e.file_name().to_string_lossy().to_string()).collect(),
        document_results,
...
pub fn format_text(report: &CalibrationReport) -> String {
    let mut lines: Vec<String> = vec![
        ...
        format!("Corpus:   {} ({} documents)", report.corpus_path, report.documents.len()),
        String::new(),
        "Documents:".to_string(),
    ];
    ...
    for doc_result in &report.document_results {
        ...
        let mut parts = vec![];
        if s.pass > 0 { parts.push(format!("{} PASS", s.pass)); }
        if s.fail > 0 { parts.push(format!("{} FAIL", s.fail)); }
        if s.manual > 0 { parts.push(format!("{} MANUAL", s.manual)); }
        if s.error > 0 { parts.push(format!("{} ERROR", s.error)); }
    ...
    let auto_cats = ["layout", "typography", "structure", "content"];
    let auto_checks: Vec<_> = report.check_frequencies.iter()
        .filter(|f| auto_cats.contains(&f.category.as_str()))
        .collect();

    for freq in &auto_checks {
        lines.push(format!("{} [{}]", freq.check_id, freq.category));
        lines.push(format!("  PASS={} FAIL={} ERROR={}", freq.pass_count, freq.fail_count, freq.error_count));
        ...
        if freq.fail_count > 0 {
            ...
            if fail_ratio >= CalibrationReport::SYSTEMIC_THRESHOLD {
                lines.push(format!("  STATUS: SYSTEMIC ({}/{})", freq.fail_count, freq.total_documents));
            } else {
                lines.push(format!("  STATUS: isolated ({}/{})", freq.fail_count, freq.total_documents));
            }
            for detail in &freq.fail_details {
                lines.push(format!("    {}", detail));
            }
        } else {
            lines.push(format!("  STATUS: clean ({}/{})", freq.pass_count, freq.total_documents));
        }
        lines.push(String::new());
    }

    let manual_checks: Vec<_> = report.check_frequencies.iter()
        .filter(|f| !auto_cats.contains(&f.category.as_str()))
        .collect();
    ...
    if !manual_checks.is_empty() {
        ...
        for freq in &manual_checks {
            lines.push(format!("{} [{}] — MANUAL review ({}/{})", freq.check_id, freq.category, freq.manual_count, freq.total_documents));
        }
        lines.push(String::new());
    }

    lines.push("=".repeat(70));
    lines.push(format!("Automated checks with >=1 FAIL: {}", report.automated_fail_count()));
    lines.push(format!("Systemic FAILs (>={}% of corpus): {}", (CalibrationReport::SYSTEMIC_THRESHOLD * 100.0) as usize, report.systemic_fail_count()));
    lines.push("=".repeat(70));
