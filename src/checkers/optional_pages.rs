...
impl Checker for CopyrightPageFormatChecker {
    ...
    fn name(&self) -> &'static str { "copyright_page_format" }

    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        let copyright_page = doc.pages.iter().find(|p| {
            p.spans.iter().any(|s| s.text.contains('©') || s.text.to_lowercase().contains("copyright"))
        });

        let page = match copyright_page {
            Some(p) => p,
            None => return CheckResult {
                check_id: String::new(), status: Status::Pass,
                evidence: vec![], detail: "No copyright page detected (optional)".to_string(),
            },
        };
        ...
        let full_text: String = page.spans.iter().map(|s| s.text.as_str()).collect::<Vec<_>>().join(" ");
        let has_year = full_text.split_whitespace().any(|w| {
        ...
        if !has_copyright_symbol || !has_year {
            violations.push(EvidenceItem {
                ...
                excerpt: Some("Copyright symbol (©) or year not found on copyright page".to_string()),
            });
        }
        ...
        if violations.is_empty() {
            CheckResult {
                ...
                detail: format!("Copyright page format correct ({} lines centered)", lines.len()),
            }
        } else {
            CheckResult {
                check_id: String::new(), status: Status::Fail,
                detail: format!("{}/{} lines not centered on copyright page", off_center_count, lines.len()),
                evidence: violations,
            }
        }
    }
...
mod tests {
    ...
    fn test_copyright_page_centered_pass() {
        let center = 306.0;
        let doc = Document { pages: vec![Page {
            page_number: 3, width: 612.0, height: 792.0,
            spans: vec![
