...
fn body_style(doc: &Document) -> (String, f32) {
    ...
    for page in &doc.pages[start..end] {
        for s in &page.spans {
            ...
            *sizes.entry((s.font_size * 10.0).round() as i32).or_insert(0) += 1;
        }
    }
    let family = fams.iter().max_by_key(|(_, c)| *c).map(|(k, _)| k.clone()).unwrap_or_default();
    let sz = sizes.iter().max_by_key(|(_, c)| *c).map(|(k, _)| *k).unwrap_or(0);
    (family, sz as f32 / 10.0)
}

fn page_center(page: &crate::document::Page) -> f32 { page.width / 2.0 }

fn line_center(spans: &[&&crate::document::TextSpan]) -> f32 {
    let left = spans.iter().map(|s| s.bbox.2).fold(f32::MAX, |a, b| a.min(b));
    let right = spans.iter().map(|s| s.bbox.3).fold(f32::MIN, |a, b| a.max(b));
    (left + right) / 2.0
}
...
impl Checker for ReferencesFontChecker {
    fn category(&self) -> &'static str { "typography" }
    fn name(&self) -> &'static str { "references_font_consistent" }
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        if ref_pages.is_empty() {
            return CheckResult { check_id: String::new(), status: Status::Error, evidence: vec![],
        ...
        if violations.is_empty() {
            CheckResult { check_id: String::new(), status: Status::Pass, evidence: vec![],
                detail: format!("References font matches body ({} {:.0}pt)", body_fam, body_sz) }
        } else {
            CheckResult { check_id: String::new(), status: Status::Fail,
                detail: format!("{} reference(s) with mismatched font", violations.len()), evidence: violations }
        }
    }
}
...
fn get_chapter_heading_style(doc: &Document) -> Option<(String, f32, bool)> {
    ...
    for page in &doc.pages {
        ...
        if !(low.contains("chapter ") || low.contains("appendix ")) { continue; }

        for s in &page.spans {
            let (top, bottom, _x0, _x1) = s.bbox;
            let low_s = s.text.to_lowercase();
            if !low_s.contains("chapter") && !low_s.contains("appendix") { continue; }
            if bottom >= page.height - 53.0 || top < 36.0 { continue; }
            *fams.entry(normalize_family(&s.font_name)).or_insert(0) += 1;
            *sizes.entry((s.font_size * 10.0).round() as i32).or_insert(0) += 1;
            if s.is_bold { bold_count += 1; }
        }
    }

    if fams.is_empty() { return None; }
    let fam = fams.iter().max_by_key(|(_, c)| *c).map(|(k, _)| k.clone())?;
...
impl Checker for ReferencesHeadingChecker {
    ...
    fn name(&self) -> &'static str { "references_heading_format" }

    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        let ch_style = match get_chapter_heading_style(doc) {
            Some(s) => s,
            None => return CheckResult { check_id: String::new(), status: Status::Error,
                evidence: vec![], detail: "No chapter headings found for comparison".to_string() },
        };
        ...
        let ref_heading = page.spans.iter()
            .find(|s| {
                let (top, bottom, _x0, _x1) = s.bbox;
                let low = s.text.to_lowercase();
                bottom <= page.height - 53.0 && top >= 36.0 && (low.contains("references") || low.contains("bibliography") || low.contains("works cited"))
            });
        ...
        match ref_heading {
            Some(s) => {
                ...
                if fam == ch_fam && (s.font_size - ch_sz).abs() <= 1.5 && s.is_bold == ch_bold {
                    CheckResult { check_id: String::new(), status: Status::Pass, evidence: vec![],
                        detail: format!("References heading matches chapter style ({}, {:.0}pt)", ch_fam, ch_sz) }
                } else {
                    CheckResult { check_id: String::new(), status: Status::Fail,
                        detail: format!("References heading font differs: {} {:.0}pt (chapters: {} {:.0}pt)",
                            s.font_name, s.font_size, ch_fam, ch_sz),
                        evidence: vec![EvidenceItem { page: ref_pages[0], bbox: Some(s.bbox),
                            excerpt: Some(s.text.clone()) }] }
                }
            }
            None => CheckResult { check_id: String::new(), status: Status::Error, evidence: vec![],
...
impl Checker for CvHeadingChecker {
    fn category(&self) -> &'static str { "structure" }
    fn name(&self) -> &'static str { "cv_heading_format" }
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        let cv_page = doc.pages.iter().rev().find(|p| {
            let text: String = p.spans.iter().map(|s| s.text.as_str()).collect::<Vec<_>>().join(" ");
            let low = text.to_lowercase();
            low.contains("curriculum vitae") || (low.contains("curriculum") && low.contains("vitae"))
        });

        match cv_page {
            Some(p) => {
                let cv_heading = p.spans.iter()
                    .filter(|s| {
                        let (top, bottom, _x0, _x1) = s.bbox;
                        let low = s.text.to_lowercase();
                        bottom <= p.height - 53.0 && top >= 36.0
                            && (low.contains("curriculum") || low.contains("vitae"))
                    })
                    .min_by(|a, b| a.bbox.0.partial_cmp(&b.bbox.0).unwrap_or(std::cmp::Ordering::Equal));
                match cv_heading {
                    Some(s) => {
                        let fam = normalize_family(&s.font_name);
                        if fam == ch_fam && (s.font_size - ch_sz).abs() <= 1.5 {
                            CheckResult { check_id: String::new(), status: Status::Pass, evidence: vec![],
                                detail: format!("CV heading matches chapter style ({}, {:.0}pt)", ch_fam, ch_sz) }
                        } else {
                            CheckResult { check_id: String::new(), status: Status::Fail,
                                detail: format!("CV heading font differs: {} {:.0}pt (chapters: {} {:.0}pt)",
                                    s.font_name, s.font_size, ch_fam, ch_sz),
                                evidence: vec![EvidenceItem { page: p.page_number, bbox: Some(s.bbox),
                                    excerpt: Some(s.text.clone()) }] }
                        }
                    }
                    None => CheckResult { check_id: String::new(), status: Status::Error,
                        evidence: vec![], detail: "CV heading not found".to_string() },
                }
            }
            None => CheckResult { check_id: String::new(), status: Status::Pass, evidence: vec![],
...
impl Checker for CvNamePositionChecker {
    fn category(&self) -> &'static str { "structure" }
    fn name(&self) -> &'static str { "cv_name_position" }
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        let page = match cv_page {
            Some(p) => p,
            None => return CheckResult { check_id: String::new(), status: Status::Pass,
                evidence: vec![], detail: "No CV page detected (optional)".to_string() },
        };

        if page.spans.len() < 2 {
            return CheckResult { check_id: String::new(), status: Status::Error,
                evidence: vec![], detail: "Not enough text on CV page".to_string() };
        }

        let mut non_empty: Vec<&crate::document::TextSpan> = page.spans.iter()
            .filter(|s| {
                let (top, _bottom, _x0, _x1) = s.bbox;
                !s.text.trim().is_empty() && top >= 72.0
            })
            .collect();
        non_empty.sort_by(|a, b| a.bbox.0.partial_cmp(&b.bbox.0).unwrap_or(std::cmp::Ordering::Equal));

        let first_top = non_empty[0].bbox.0;
        let name_spans: Vec<&&crate::document::TextSpan> = non_empty.iter()
        ...
        if is_centered || is_left {
            let pos = if is_centered { "centered" } else { "left-aligned" };
            CheckResult { check_id: String::new(), status: Status::Pass, evidence: vec![],
                detail: format!("CV name is {}", pos) }
        } else {
            let name_text: String = name_spans.iter().map(|s| s.text.trim()).collect::<Vec<_>>().join(" ");
            CheckResult { check_id: String::new(), status: Status::Fail,
                detail: format!("CV name not centered or left-aligned: \"{}\"", name_text),
                evidence: name_spans.iter().map(|s| EvidenceItem {
                    page: page.page_number, bbox: Some(s.bbox), excerpt: Some(s.text.clone()),
                }).collect() }
        }
    }
}
...
impl Checker for AbstractTextCenteredChecker {
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        if non_empty.len() < 2 {
            return CheckResult { check_id: String::new(), status: Status::Error,
                evidence: vec![], detail: "Not enough text on abstract page".to_string() };
        }

        let body_sort: Vec<(i32, f32)> = {
            let mut tops: Vec<(i32, f32)> = non_empty.iter().map(|s| {
                (s.bbox.0.round() as i32, s.font_size)
            }).collect();
        ...
        if body_sort.len() < 2 {
            return CheckResult { check_id: String::new(), status: Status::Error,
                evidence: vec![], detail: "Not enough lines on abstract page".to_string() };
        }

        let first_top = body_sort[0].0 as f32;
        let second_top = body_sort[1].0 as f32;
        let name_lines: Vec<i32> = {
            let mut group: std::collections::BTreeMap<i32, Vec<&&crate::document::TextSpan>> = std::collections::BTreeMap::new();
            for s in &non_empty {
                let tk = s.bbox.0.round() as i32;
                if (tk as f32 - first_top).abs() < 6.0 { group.entry(tk).or_default().push(s); }
            }
        ...
        let title_lines: Vec<i32> = {
            ...
            for s in &non_empty {
                let tk = s.bbox.0.round() as i32;
                if (tk as f32 - second_top).abs() < 6.0 { group.entry(tk).or_default().push(s); }
            }
            let mut ks: Vec<i32> = group.keys().copied().collect();
            ks.sort();
            ks
        ...
        for top in &name_lines {
            let spans: Vec<&&crate::document::TextSpan> = non_empty.iter()
        ...
        if violations.is_empty() {
            ...
            CheckResult { check_id: String::new(), status: Status::Fail,
                detail: format!("{}/2 header lines not centered", violations.len()), evidence: violations }
        }
    }
}
...
impl Checker for AbstractWordCountChecker {
    fn category(&self) -> &'static str { "content" }
...
impl Checker for AbstractTitleFormatChecker {
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        let mut tops: Vec<(i32, f32)> = non_empty.iter().map(|s| (s.bbox.0.round() as i32, s.font_size)).collect();
        tops.sort_by_key(|(t, _)| *t);
        tops.dedup_by_key(|(t, _)| *t);

        let second_top = if tops.len() > 1 { tops[1].0 as f32 } else { tops[0].0 as f32 };
        let title_spans: Vec<&&crate::document::TextSpan> = non_empty.iter()
            .filter(|s| (s.bbox.0 - second_top).abs() < 6.0)
            .collect();

        let title_text: String = title_spans.iter().map(|s| s.text.trim()).collect::<Vec<_>>().join(" ");
        ...
        let first_letter_upper = alpha.first().map_or(true, |c| c.is_uppercase());
...
mod tests {
    ...
    fn span(text: &str, top: f32, fs: f32, fam: &str) -> crate::document::TextSpan {
        crate::document::TextSpan {
            text: text.to_string(), font_name: fam.to_string(), font_size: fs,
            bbox: (top, top + fs, 100.0, 200.0), is_bold: false, is_italic: false, color: None,
        }
    }
    ...
    fn body_pages(count: usize) -> Vec<Page> {
        ...
        for i in 1..=count {
            ...
            for j in 0..25 {
                spans.push(span("body text line", 72.0 + j as f32 * 24.0, 12.0, "TimesNewRoman"));
            }
            pages.push(Page { page_number: i, width: 612.0, height: 792.0, spans, images: vec![], paths: vec![] });
        }
        pages
    }

    fn page_with_heading(pn: usize, spans: Vec<crate::document::TextSpan>) -> Page {
        Page { page_number: pn, width: 612.0, height: 792.0, spans, images: vec![], paths: vec![] }
    }
