...
fn compute_body_style(doc: &Document) -> (String, f32) {
    ...
    let family = families.iter().max_by_key(|(_, c)| *c).map(|(k, _)| k.clone()).unwrap_or_default();
    let size_key = sizes.iter().max_by_key(|(_, c)| *c).map(|(k, _)| *k).unwrap_or(0);
...
impl Checker for FootnotesFontChecker {
    ...
    fn name(&self) -> &'static str { "footnotes_font_consistent" }
    ...
    fn check(&self, doc: &Document, _params: &Value) -> CheckResult {
        ...
        for page in &doc.pages {
            if page.spans.is_empty() { continue; }
            let body_bottom = page.spans.iter()
                .filter(|s| {
                    let (top, bottom, _x0, _x1) = s.bbox;
                    top >= 72.0 && bottom <= page.height - 72.0
            ...
            if body_bottom < 100.0 || body_bottom > page.height - 50.0 { continue; }
                ...
                || page.spans.iter().any(|s| {
                    ...
                    top > body_bottom + 2.0
                        ...
                        && (t.starts_with(|c: char| c.is_ascii_digit())
                            || t.starts_with('*')
                            || t.contains("footnote") || t.contains("note"))
                });

            if !footnote_detected { continue; }

            let footnote_spans: Vec<&crate::document::TextSpan> = page.spans.iter()
                .filter(|s| {
                    let (top, bottom, _x0, _x1) = s.bbox;
            ...
            for s in &footnote_spans {
                ...
                if fam_norm != body_family {
                    violations.push(EvidenceItem {
                        page: page.page_number, bbox: Some(s.bbox),
                        excerpt: Some(format!(
        ...
        if violations.is_empty() {
            CheckResult {
                ...
                detail: format!("Footnote text consistent with body font ({} {:.0}pt)", body_family, body_size),
            }
        } else {
            CheckResult {
...
mod tests {
    ...
    fn span(text: &str, top: f32, font_size: f32, font_name: &str) -> crate::document::TextSpan {
        crate::document::TextSpan {
            text: text.to_string(), font_name: font_name.to_string(), font_size,
            bbox: (top, top + font_size, 100.0, 200.0), is_bold: false, is_italic: false, color: None,
        }
    }
    ...
    fn body_page(pn: usize) -> Page {
        ...
        for i in 0..25 {
            spans.push(span("body text line", 72.0 + i as f32 * 24.0, 12.0, "TimesNewRoman"));
        }
    ...
    fn test_footnotes_pass_no_footnotes() {
        ...
        for i in 1..12 { pages.push(body_page(i)); }
    ...
    fn test_footnotes_pass_good_footnote() {
        ...
        for i in 1..11 { pages.push(body_page(i)); }
        let mut footnote_page = body_page(11);
        let body_bottom = 72.0 + 24.0 * 24.0 + 12.0; // last body line bottom
        footnote_page.spans.push(span("1. See Smith (2020)", body_bottom + 8.0, 10.0, "TimesNewRoman"));
    ...
    fn test_footnotes_fail_wrong_font() {
        ...
        footnote_page.spans.push(span("1. See Smith (2020)", body_bottom + 8.0, 10.0, "Arial"));
        pages.push(footnote_page);
        let doc = Document { pages };
        let r = FootnotesFontChecker.check(&doc, &Value::Null);
    ...
    fn test_footnotes_fail_too_small() {
        ...
        let mut footnote_page = body_page(11);
        let body_bottom = 72.0 + 24.0 * 24.0 + 12.0;
        footnote_page.spans.push(span("1. tiny text", body_bottom + 8.0, 8.0, "TimesNewRoman"));
        pages.push(footnote_page);
        let doc = Document { pages };
        let r = FootnotesFontChecker.check(&doc, &Value::Null);
        assert_eq!(r.status, Status::Fail, "{}", r.detail);
