...
fn parse_measurement(value: &str) -> Result<f32, String> {
    ...
    if let Some(inches) = value.strip_suffix("in") {
        inches.trim().parse::<f32>().map(|v| v * 72.0).map_err(|e| format!("Invalid inches: {}", e))
    } else if let Some(pts) = value.strip_suffix("pt") {
        pts.trim().parse::<f32>().map_err(|e| format!("Invalid points: {}", e))
    } else {
...
fn right_margin_ptile(spans: &[&crate::document::TextSpan], page_width: f32) -> Option<f32> {
    let mut margins: Vec<i32> = spans.iter()
        .map(|s| (page_width - s.bbox.3).round() as i32)
        .filter(|&m| m >= 0)
        .collect();
    if margins.is_empty() { return None; }
    margins.sort();
    let idx = (margins.len() as f32 * 0.05) as usize;
    Some(margins[idx.min(margins.len() - 1)] as f32)
}

fn check_edge(label: &str, values: &[f32], required: f32, tolerance: f32) -> Option<(bool, String)> {
    ...
    let direction = if avg < lower { " too narrow" } else if avg > upper { " too wide" } else { "" };
    Some((pass, format!(
        "Avg {}: {:.0}pt ({:.2}in) — range [{:.2}in–{:.2}in]. {}{}",
        label, avg, avg / 72.0, lower / 72.0, upper / 72.0, status, direction
    )))
}
...
impl Checker for MarginsChecker {
    ...
    fn name(&self) -> &'static str { "margins" }
    ...
    fn check(&self, doc: &Document, params: &Value) -> CheckResult {
        ...
        let bottom_req = parse_measurement(params["bottom"].as_str().unwrap_or("1in")).unwrap_or(72.0);
        let left_req = parse_measurement(params["left"].as_str().unwrap_or("1.25in")).unwrap_or(90.0);
        let right_req = parse_measurement(params["right"].as_str().unwrap_or("1.25in")).unwrap_or(90.0);
        let tolerance = parse_measurement(params["tolerance"].as_str().unwrap_or("0.125in")).unwrap_or(9.0);
        ...
        for page in &doc.pages {
            let body: Vec<&crate::document::TextSpan> = page.spans.iter()
                .filter(|s| {
            ...
            if let Some(e) = left_edge_ptile(&body) { left_edges.push(e); }
            if let Some(e) = right_margin_ptile(&body, page.width) { right_margins.push(e); }
            if let Some(s) = body.iter().min_by(|a, b| a.bbox.0.partial_cmp(&b.bbox.0).unwrap_or(std::cmp::Ordering::Equal)) {
                page_first_tops.push(s.bbox.0);
            }
            if let Some(s) = body.iter().max_by(|a, b| a.bbox.1.partial_cmp(&b.bbox.1).unwrap_or(std::cmp::Ordering::Equal)) {
        ...
        if left_edges.is_empty() {
            return CheckResult { check_id: String::new(), status: Status::Error,
                evidence: vec![], detail: "Insufficient body text to measure margins".to_string() };
        }
        ...
        for (label, values, req) in [
            ...
            if let Some((pass, line)) = check_edge(label, values, req, tolerance) {
                lines.push(line);
                if !pass {
                    violations.push(EvidenceItem { page: 0, bbox: None,
                        excerpt: Some(format!("{} {}pt outside [{}-{}pt]", label, mean(values) as i32, (req-tolerance) as i32, (req+tolerance) as i32)),
                    });
                }
            }
        }
        ...
        if violations.is_empty() {
            ...
            CheckResult { check_id: String::new(), status: Status::Fail,
                detail: lines.join("; "), evidence: violations }
        }
    }
}
...
impl Checker for MarginSymmetryChecker {
    fn category(&self) -> &'static str { "layout" }
    fn name(&self) -> &'static str { "margin_symmetry" }

    fn check(&self, doc: &Document, params: &Value) -> CheckResult {
        let threshold = parse_measurement(params["threshold"].as_str().unwrap_or("0.25in")).unwrap_or(18.0);
        ...
        for page in &doc.pages {
            ...
            if diff.abs() > threshold {
                ...
                evidence.push(EvidenceItem { page: page.page_number, bbox: None,
                    excerpt: Some(format!("asymmetry {:.0}pt ({:.2}in): L={:.0}pt R={:.0}pt ({})", diff.abs(), diff.abs() / 72.0, left_mean, right_mean, direction)),
                });
            }
        }
        ...
        if asymmetrical_pages == 0 {
            ...
            CheckResult { check_id: String::new(), status: Status::Fail,
                detail: format!("{} page(s) have asymmetric margins (threshold: {:.0}pt / {:.2}in)", asymmetrical_pages, threshold, threshold / 72.0),
                evidence }
        }
    }
}
...
mod tests {
    ...
    fn make_span(text: &str, bbox: (f32, f32, f32, f32)) -> TextSpan {
        TextSpan { text: text.to_string(), font_name: "TimesNewRoman".to_string(), font_size: 12.0,
            bbox, is_bold: false, is_italic: false, color: None }
    }

    fn multi_page_doc(pages_data: Vec<Vec<(f32, f32, f32, f32)>>) -> Document {
        Document { pages: pages_data.iter().enumerate().map(|(i, spans)| {
            Page { page_number: i + 1, width: 612.0, height: 792.0,
                spans: spans.iter().map(|&b| make_span("body text line here", b)).collect(),
    ...
    fn body_spans(count: usize, left_x: f32, right_x: f32, top_start: f32, gap: f32) -> Vec<(f32, f32, f32, f32)> {
        (0..count).map(|i| {
