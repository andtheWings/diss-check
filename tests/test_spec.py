...
def test_load_minimal_iu_spec():
    ...
    assert len(spec.checks) == 27
    assert all(c.category in ("layout", "typography", "structure", "content", "human") for c in spec.checks)
    assert spec.constants["degree"] == "Doctor of Philosophy"
